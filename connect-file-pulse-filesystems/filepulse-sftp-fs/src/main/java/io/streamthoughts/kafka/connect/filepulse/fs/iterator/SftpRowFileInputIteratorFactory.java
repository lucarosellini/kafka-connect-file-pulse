...
public class SftpRowFileInputIteratorFactory implements FileInputIteratorFactory {
    private static final Logger LOG = LoggerFactory.getLogger(SftpRowFileInputIteratorFactory.class);
    ...
    public FileInputIterator<FileRecord<TypedStruct>> newIterator(URI objectURI) {
        ...
        return new SftpRowFileInputIteratorBuilder()
                ...
                .withReaderSupplier(new SFTPBufferedReaderSupplier(objectURI))
                .build();
    }

    class SFTPBufferedReaderSupplier implements Supplier<NonBlockingBufferReader> {

        private final URI objectURI;

        public SFTPBufferedReaderSupplier(URI objectURI) {
            this.objectURI = objectURI;
        }

        @Override
        public NonBlockingBufferReader get() {

            NonBlockingBufferReader br;
            try {
                br = new NonBlockingBufferReader(
                ...
                throw new ReaderException("[SFTP] Cannot get InputStream", e);
            }
            return br;
        }
    }
